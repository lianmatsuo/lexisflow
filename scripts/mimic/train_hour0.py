#!/usr/bin/env python3
"""Train the hour-0 IID Forest-Flow model for initial patient states.
...
def main():
    ...
    if not Path(input_path).exists():
        ...
        print("Run scripts/prepare_hour0.py first.")
        sys.exit(1)

    if not Path(preprocessor_path).exists():
        print(f"\nError: {preprocessor_path} not found!")
        print("Run scripts/fit_hour0_preprocessor.py first.")
        sys.exit(1)
