...
1. ✅ **CSV path and format standardization**
   - Unified under `packages/data/processed/`
...
2. ✅ **Feature engineering and preprocessing workflow**
   ...
   - Full-data preprocessor fitting (`01b_fit_preprocessor.py`)
...
3. ✅ **Model implementations**
   ...
   - Unified training interface (`02_train_model.py --model-type {forest-flow,hs3f}`)

4. ✅ **Hour-0 model workflow**
   - `00_prepare_hour0_data.py`
   - `00b_fit_hour0_preprocessor.py`
   - `00c_train_hour0_model.py`
   - `03_generate_synthetic.py --use-hour0` integration completed
...
6. ✅ **Sweep infrastructure**
   ...
   - Visualization suite (`plot_sweep_results.py`)

7. ✅ **Temporal coherence metrics implementation**
   - Added trajectory-level metrics in `packages/evaluation/trajectory_metrics.py`
   ...
   - Covered by dedicated unit tests (`packages/evaluation/tests/test_trajectory_metrics.py`)

8. ✅ **CTGAN baseline integration (implementation stage)**
   - Added `CTGANAdapter` (`packages/models/ctgan_adapter.py`)
   - Integrated into model registry and sweep training paths
   - Added matched-comparison runner (`scripts/run_matched_backbone_comparison.py`) for HS3F vs ForestFlow vs CTGAN at a matched cell

9. ✅ **Testing**
   - 54 unit tests in `packages/*/tests`
