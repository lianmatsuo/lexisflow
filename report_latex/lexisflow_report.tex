...
This project applies tree-based flow matching to the generation of \textbf{sequential ICU patient trajectories} derived from the MIMIC-III clinical database \cite{johnson2016mimic}. Unlike prior work that evaluates ForestFlow and HS3F on static, single-row tabular benchmarks, this project extends the approach to an \textbf{autoregressive setting}: a two-stage architecture where an independent and identically distributed (IID) hour-0 model generates initial patient states and a conditional model generates subsequent hourly transitions given lagged dynamics and static covariates. Two model backbones are implemented and compared: ForestFlow (all-continuous flow) and HS3F (heterogeneous sequential generation with separate continuous and categorical heads).

The objective is to investigate whether tree-based flow matching can produce synthetic ICU trajectories with useful \textbf{fidelity}, \textbf{downstream utility}, and \textbf{privacy diagnostics} while training efficiently on CPUs. The focus is technical feasibility and limitation-mapping rather than deployment claims.

Evaluation follows a multi-axis protocol: (1) \emph{fidelity} via Kolmogorov--Smirnov statistics, correlation preservation, and clinical range violation rates; (2) \emph{utility} via multi-task Train-on-Synthetic, Test-on-Real (TSTR) evaluation covering patient-level mortality prediction and ICU length-of-stay (LOS) prediction; and (3) \emph{privacy} via Distance-to-Closest-Record diagnostics and DOMIAS-style membership inference attacks.

%==============================================================================
\section{Thesis}
%==============================================================================

This dissertation argues that tree-based flow matching, originally formulated for static tabular data, can be adapted to sequential ICU trajectory synthesis on MIMIC-III through a two-stage autoregressive factorisation. Under the evaluated settings, the system shows credible feasibility with measurable but incomplete utility transfer and privacy diagnostics that remain informative rather than release-assuring. The dominant unresolved weakness is minority-class collapse, which limits practical adequacy despite positive feasibility evidence.
...
\begin{enumerate}
    \item \textbf{Feasibility:} Can tree-based flow matching, originally evaluated on static tabular benchmarks, be extended to generate realistic sequential ICU trajectories via autoregressive conditioning?
    \item \textbf{Heterogeneous features:} Within the evaluated sweep settings and matched comparison cells, does HS3F's explicit separation of continuous and categorical generation heads improve fidelity over ForestFlow's all-continuous approach on mixed-type clinical data?
    \item \textbf{Utility and privacy:} Do synthetic trajectories preserve sufficient downstream predictive signal (TSTR) while maintaining acceptable privacy-risk indicators under proximity-based and attack-based diagnostics?
\end{enumerate}

Throughout the report, a central distinction is maintained between \emph{research feasibility} and \emph{practical adequacy}: a configuration may generate coherent trajectories and transfer some predictive signal while still falling short of release or operational standards.
