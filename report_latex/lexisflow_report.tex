...
Synthetic tabular generation offers a promising path to address privacy, scarcity, and imbalance, but current methods still struggle with full clinical complexity. Generative Adversarial Networks (GANs) are prone to mode collapse and can lose rare patterns; Variational Autoencoders (VAEs) may oversmooth heterogeneous distributions; and neural diffusion models often require heavier GPU resources while still needing careful handling of categorical and structural constraints.

Recent work on tree-based flow matching, notably ForestFlow \cite{jolicoeur2023forest} and Heterogeneous Sequential Feature Forest Flow (HS3F) \cite{akazan2024hs3f}, provides evidence that gradient-boosted decision-tree ensembles can serve as the flow-field approximator in place of neural networks for mixed-type tabular synthesis. Trees partition feature space into locally homogeneous regions that better match tabular structure, while avoiding much of the computational overhead of deep backbones.
...
\begin{description}[leftmargin=0cm,labelsep=0.6em]
    ...
    \item[Backbone implementation and comparison:] Implement both ForestFlow and HS3F within a unified training and evaluation interface to enable empirical comparison of all-continuous versus heterogeneous sequential generation on mixed-type ICU data.
...
\begin{description}[leftmargin=0cm,labelsep=0.6em]
    ...
    \item[Heterogeneous features:] Within the evaluated sweep settings and matched comparison cells, does HS3F's explicit separation of continuous and categorical generation heads improve fidelity over ForestFlow's all-continuous approach on mixed-type clinical data?
