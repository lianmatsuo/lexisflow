...
through a MIMIC-Extract style workflow \cite{wang2020mimicextract}. The canonical training input for autoregressive modeling is \code{src/synth_gen/data/processed/autoregressive\_data.csv}.
Hour-0 training uses \code{src/synth_gen/data/processed/hour0\_data.csv}.
...
The autoregressive table is built in \code{src/synth_gen/data/autoregressive.py} by sorting on \code{(subject\_id, hours\_in)} and, for each dynamic column, computing \code{groupby(subject\_id)[col].shift(lag)} with \code{lag=1}.
...
Feature typing is centralized in \code{src/synth_gen/data/feature\_utils.py} and consumed by \code{scripts/fit\_autoregressive\_preprocessor.py}.
...
\code{scripts/fit\_autoregressive\_preprocessor.py} performs chunked fitting on the full autoregressive CSV via repeated \code{partial\_fit()} calls followed by \code{finalize\_fit()}, then persists the artifact at \code{models/preprocessor\_full.pkl}.
...
The \code{ForestFlow} class in \code{src/synth_gen/models/forest\_flow.py} implements conditional flow matching with XGBoost regressors.
Core interface:
\begin{itemize}\tightlist
...
The \code{HS3F} class in \code{src/synth_gen/models/hs3f.py} follows heterogeneous sequential generation \cite{akazan2024hs3f}:
\begin{itemize}\tightlist
