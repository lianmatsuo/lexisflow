...
This chapter establishes the architectural decisions that shape the generator and evaluation interface. Command-line driving, configuration defaults, resumable sweep drivers, and repository layout are documented in the User Guide (companion appendix). Chapter~\ref{ch:implementation} records only what is needed to interpret methods, metrics, and reported results.
...
\label{sec:mimic-preprocess-contract}

The operational pipeline starts from hourly ICU tables derived from MIMIC-III \cite{johnson2016mimic}
through this project's preprocessing path: patient-level cohort filtering, hourly
binning/aggregation, static/dynamic feature split, and lag-feature table
construction, following MIMIC-Extract representation conventions
\cite{wang2020mimicextract}. The system uses separate hour-0 and autoregressive tables under a shared preprocessing contract.
...
Lag-1 conditioning leaves the first timestep per patient without prior history; this undefined lag state is represented with a shared sentinel value ($-1.0$) so the training matrix remains dense and rectangular. The same marker is used when prior-hour values are missing, so ``no prior timestep'' and ``prior value missing'' are not separated at this stage.
...
The preprocessor stores both semantic role definitions (target versus condition columns) and transformed-space index mappings. This is necessary because transformed output is grouped by feature type rather than by semantic generation order.
...
In the current study design, trajectories are generated on a fixed 48-hour horizon, which simplifies comparability across runs but limits stay-length realism.

\paragraph{Sweep-level design.}
The $(n_t, n_{\text{noise}})$ grid, transformed-matrix caching, and canonical sweep result schema are operational rather than architectural; a short summary sufficient for reading results appears in Chapter~\ref{ch:implementation} (\ref{sec:unified-sweep}). Full CLI options, profiles, and driver controls are in the User Guide (companion appendix).

\subsection{Design Implications for Evaluation Outcomes}

Several downstream failure modes in Chapter~7 follow directly from design choices documented here. First, the fixed 48-hour rollout horizon constrains stay-length fidelity and contributes to the LOS utility gap. Second, shared lag sentinels for both lag-0 and prior-hour missingness introduce ambiguity in conditional context, which can degrade marginal fidelity. Third, continuous-flow handling of quantitative coordinates without explicit range constraints increases exposure to out-of-range synthetic values, which is reflected in elevated clinical range-violation rates. These links are causal design bounds rather than post-hoc interpretation.
