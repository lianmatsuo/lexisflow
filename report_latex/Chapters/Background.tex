...
\subsubsection{Second Dataset: PhysioNet Challenge 2012}
To test pipeline portability beyond a single credentialed source, this project also includes the PhysioNet/Computing in Cardiology Challenge 2012 set-A cohort \cite{silva2012challenge}. In contrast to MIMIC-III's broad hospital EHR coverage, Challenge 2012 is a narrower ICU benchmark centred on the first 48 hours, with static demographics, hourly clinical measurements, and in-hospital outcome labels. Within this dissertation, Challenge 2012 is used as the second dataset for reproducibility and cross-dataset method validation under the same downstream training and evaluation stack.

\subsection{Existing Synthetic Tabular and EHR Generators}
This project evaluates HS3F against ForestFlow in a pre-specified matched cell, with CTGAN as a practical lower-bound reference within the same data and metric pipeline; the sweep study is HS3F-only. Other families (diffusion and transformer generators) are discussed as context for trade-offs rather than as implemented competitors.
