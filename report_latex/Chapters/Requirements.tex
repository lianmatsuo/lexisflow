...
\begin{description}
    \item[\textbf{Clinical Data Custodian (CDC).}] A data engineer or clinical informatics specialist at a hospital or research institute who holds a credentialed dataset (e.g.\ MIMIC-III) and wishes to derive a shareable synthetic analogue. This actor is primarily motivated by data-governance obligations, privacy regulations such as UK GDPR and the Data Protection Act 2018 restrict direct sharing of raw patient records \cite{dataProtectionAct2018,icoResearchSafeguards}. The CDC runs the training and generation pipeline and is responsible for evaluating the privacy properties of the synthetic output before any release.
...
\begin{enumerate}
    ...
    \item The CDC runs the data preparation scripts (\texttt{prepare\_hour0}, \texttt{prepare\_autoregressive}) to build autoregressive training tables.
    \item The CDC fits the tabular preprocessor on the full dataset (\texttt{fit\_autoregressive\_preprocessor}) to capture all categorical values and numeric ranges.
    \item The CDC trains the generative model (\texttt{train\_autoregressive}) using the preferred backbone (ForestFlow or HS3F).
    \item The CDC generates a synthetic patient cohort (\texttt{generate}) at the desired scale.
...
\textbf{Acceptance target (research stage):} DCR overfitting-protection score exceeds the project-defined acceptability threshold specified in the Evaluation chapter; DOMIAS-style MIA ROC-AUC near chance level; no exact-match records in synthetic output. Meeting these targets supports cautious feasibility claims, not formal release assurance.
...
\textbf{Acceptance target (research stage):} Synthetic-trained classifier achieves ROC-AUC within 0.15 ROC-AUC of the real-trained baseline; F1 scores indicate minority-class retention.
...
\begin{enumerate}
    \item The PA receives the synthetic cohort and the evaluation artefacts (sweep log, privacy metrics) from the CDC.
...
\textbf{Acceptance target (research stage):} MIA ROC-AUC within 0.1 of 0.5; exact-match rate $<$ 0.001 between synthetic records and real training records; privacy report is documented alongside the synthetic dataset.
...
\begin{itemize}\tightlist
    ...
    \item \textbf{Computational cost}: Sweep runtime scales as $O(n_t \times n_{\text{noise}} \times d_x)$ per configuration, which at 8$\times$8 Fibonacci grid scale yields wall-clock costs between $\approx$3 minutes and $\approx$6 hours per cell; low-cost cells correspond to small $(n_t, n_{\text{noise}})$, high-cost cells to $(n_t{=}34, n_{\text{noise}}{=}34)$.
...
%==============================================================================
\section{Target Criteria and Outcome Alignment}
%==============================================================================

To avoid ambiguity, the use-case criteria above are treated as \emph{acceptance targets} for this dissertation rather than as pre-satisfied outcomes. Chapter~7 reports whether each target was met, partially met, or not met.

Based on the current evidence reported later in the dissertation:
\begin{itemize}\tightlist
    \item The utility target of being within 0.15 ROC-AUC of the real baseline was \textbf{not met} (best synthetic mortality ROC-AUC remains around 0.658 versus a real baseline around 0.82).
    \item Minority-class retention was \textbf{not met} (mortality and LOS F1 values indicate persistent minority-class collapse; see Evaluation section~\ref{sec:mortality-tstr}).
    \item Privacy diagnostics are \textbf{partially met} as feasibility evidence (no exact-match records and bounded attack scores), but they do \textbf{not} establish formal release assurance on their own.
\end{itemize}
