...
Tabular data is one of the most widely used data formats in real-world machine learning, underpinning applications in healthcare, finance, transportation, and public policy. Despite its prevalence, tabular data presents several challenges for modelling: mixed numerical and categorical variables, sparse and irregular dependencies, long-tailed distributions, and domain-specific rules that must be implicitly respected. These challenges are especially acute in healthcare analytics, where Electronic Health Records (EHRs) contain heterogeneous clinical, demographic, and laboratory measurements. Privacy regulations such as HIPAA and GDPR further restrict data access, while rare diseases and minority subpopulations lead to severe data scarcity and class imbalance. Together, these constraints make healthcare an important domain for developing synthetic tabular data generators.

Synthetic data offers a promising path to address privacy, scarcity, and imbalance. However, existing generative approaches struggle to model the full complexity of tabular datasets. GANs are prone to mode collapse, often losing rare patterns; VAEs may oversmooth heterogeneous distributions; and neural diffusion models frequently require heavy GPU resources and intricate architectural tuning to avoid violating categorical and structural constraints.

Recent work on tree-based flow matching---notably ForestFlow \cite{jolicoeur2023forest} and Heterogeneous Sequential Feature Forest Flow (HS3F) \cite{akazan2024hs3f}---shows that gradient-boosted decision-tree ensembles can replace neural networks as the function approximator within flow-matching generative pipelines. Trees partition the feature space into locally homogeneous regions that naturally reflect the structure of tabular data, offering strong inductive biases without the computational overhead of deep neural networks. By embedding these models within flow-matching pipelines, the generator inherits both flexible generative dynamics and the tabular modelling strengths of tree ensembles.

This project applies tree-based flow matching to the generation of \textbf{sequential ICU patient trajectories} derived from the MIMIC-III clinical database \cite{johnson2016mimic}. Unlike prior work that evaluates ForestFlow and HS3F on static, single-row tabular benchmarks, this project extends the approach to an \textbf{autoregressive setting}: a two-stage architecture where an IID hour-0 model generates initial patient states and a conditional model generates subsequent hourly transitions given lagged dynamics and static covariates. Two model backbones are implemented and compared: ForestFlow (all-continuous flow) and HS3F (heterogeneous sequential generation with separate continuous and categorical heads).

The objective is to investigate whether tree-based flow matching can produce synthetic ICU trajectories with sufficient \textbf{fidelity}, \textbf{downstream utility}, and \textbf{privacy preservation} while training efficiently on CPUs. Tree-based models require fewer hyperparameters and scale more gracefully than deep neural generative models, making them attractive for organisations with constrained resources or strict deployment budgets.

Evaluation follows a multi-axis protocol: (1) \emph{fidelity} via Kolmogorov--Smirnov statistics, correlation preservation, and clinical range violation rates; (2) \emph{utility} via multi-task Train-on-Synthetic, Test-on-Real (TSTR) evaluation covering patient-level mortality and ICU length-of-stay prediction; and (3) \emph{privacy} via Distance-to-Closest-Record diagnostics and DOMIAS-style membership inference attacks.

%==============================================================================
\section{Thesis}
%==============================================================================

This dissertation demonstrates that tree-based flow matching, originally formulated for static tabular data, can be extended to sequential ICU trajectory synthesis on MIMIC-III via a two-stage autoregressive factorisation, yielding measurable but incomplete utility transfer (best single-seed synthetic TSTR ROC-AUC $\approx 0.74$ versus a real-model baseline of $\approx 0.83$) and diagnostic-level privacy separation (membership inference ROC-AUC near chance across all evaluated configurations, range $0.48$--$0.50$; zero exact-match records), while exposing minority-class collapse as the principal outstanding failure mode.
...
\paragraph{Aim.}
The aim of this project is to develop and evaluate a tree-based flow-matching framework for synthesising realistic sequential ICU patient trajectories from the MIMIC-III clinical database \cite{johnson2016mimic}, assessing whether this approach can produce synthetic data with sufficient fidelity, downstream clinical utility, and privacy preservation to support data-sharing and model-development workflows in resource-constrained healthcare research settings.

\paragraph{Objectives.}
This aim is operationalised through four concrete objectives:

\begin{enumerate}
    \item \textbf{Domain adaptation:} Extend ForestFlow \cite{jolicoeur2023forest} and HS3F \cite{akazan2024hs3f}---tree-based flow-matching models originally evaluated on static tabular benchmarks---to sequential clinical data via an autoregressive two-stage architecture comprising an IID hour-0 model for initial patient states and a conditional transition model for subsequent hourly dynamics.
    \item \textbf{Backbone implementation and comparison:} Implement both ForestFlow and HS3F within a unified training and evaluation interface to enable empirical comparison of all-continuous versus heterogeneous sequential generation on mixed-type ICU data.
    \item \textbf{Multi-axis evaluation framework:} Develop a reproducible evaluation stack covering (i) distributional fidelity via Kolmogorov--Smirnov statistics, correlation preservation, and clinical range violation rates; (ii) downstream utility via multi-task Train-on-Synthetic, Test-on-Real (TSTR) evaluation; and (iii) privacy risk via Distance-to-Closest-Record diagnostics and DOMIAS-style membership inference attacks \cite{breugel2023domias}.
    \item \textbf{Systematic hyperparameter investigation:} Evaluate the effect of key flow-matching hyperparameters ($n_t$, $n_{\text{noise}}$) on synthetic data quality, utility, and computational cost via a reproducible, resumable grid sweep.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Feasibility:} Can tree-based flow matching, originally designed for static tabular data, be extended to generate realistic sequential ICU trajectories via autoregressive conditioning?
    \item \textbf{Heterogeneous features:} Does HS3F's explicit separation of continuous and categorical generation heads improve fidelity over ForestFlow's all-continuous approach for mixed-type clinical data?
    \item \textbf{Utility and privacy:} Do synthetic trajectories preserve sufficient downstream predictive signal (TSTR) while maintaining acceptable privacy risk as measured by proximity-based and attack-based diagnostics?
\end{enumerate}

This project does not propose new generative algorithms.
Rather, it contributes an \textbf{application-level integration} of existing tree-based flow-matching methods into a new domain (sequential clinical data) and a new problem structure (autoregressive trajectory generation).
...
\bibitem{ieee29148Requirements2018}
