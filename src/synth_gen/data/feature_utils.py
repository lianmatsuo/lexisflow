...
# ``REMOVE_IF_PANEL`` rows from ``results/column_removal_recommendations.csv``:
# 1–5% non-null targets; dropped when using the default tighter ICU-style panel.
SPARSE_REMOVE_FEATURES_IF_PANEL = frozenset(
    {
        "pH_urine_mean",
        "Monocytes_mean",
        "Lymphocytes_mean",
        ...
        "Troponin-T_mean",
        "Albumin_mean",
        "Positive_end-expiratory_pressure_mean",
        "Alkaline_phosphate_mean",
        "Alanine_aminotransferase_mean",
        "Asparate_aminotransferase_mean",
        "Bilirubin_mean",
        "Lactic_acid_mean",
        "Lactate_mean",
        "Weight_mean",
        "Cardiac_Output_Thermodilution_mean",
        "Pulmonary_Artery_Pressure_mean_mean",
        "Plateau_Pressure_mean",
        "Tidal_Volume_Spontaneous_mean",
        "Potassium_serum_mean",
        "Systemic_Vascular_Resistance_mean",
        "CO2_mean",
        "Cardiac_Index_mean",
        "Partial_pressure_of_oxygen_mean",
        "Tidal_Volume_Set_mean",
        "Fraction_inspired_oxygen_mean",
        "Respiratory_rate_Set_mean",
        "Calcium_ionized_mean",
    }
)
...
def columns_to_drop_default_feature_pruning(columns: Iterable[str]) -> list[str]:
    ...
    2) Fixed sparse-feature removal lists from
       ``results/column_removal_recommendations.csv``: all ``REMOVE`` targets
       (37) plus ``REMOVE_IF_PANEL`` targets, each with matching ``_lag1`` when
    ...
    sparse_targets = SPARSE_REMOVE_FEATURES | SPARSE_REMOVE_FEATURES_IF_PANEL
