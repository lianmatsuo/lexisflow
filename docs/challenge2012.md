# PhysioNet Challenge 2012 Reproducibility Guide
...
- **Size:** 4,000 patients x 48 hourly bins (189,120 rows after gridding)
...
- Patient-disjoint 60/20/20 train/test/holdout split inside
  `prepare_autoregressive.py` guards the privacy overfitting-protection metric
  from train/holdout overlap.
- Imputation (ffill->bfill within patient -> global median) is required for the
  flow-matching target matrix and downstream `MinMaxScaler` /
